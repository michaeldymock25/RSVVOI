...
Contains statistical models required by functions defined in *R/*. The models are implemented as *.stan* files.
...
Contains R scripts that define documented functions including *trans_functions.R*, *cea_functions.R*, *trial_functions.R*, *voi_functions.R* and *misc_functions.R*.

### Run

Contains R scripts that use the functions defined in *R/*. The *make_mixing_matrix.R* script generates the mixing matrix and aggregated Australian population data files stored in *Data/*. The *make_hyper_parms.R* script generates the statistical model hyperparameters stored in *Parameters/*. The *burn_trans_model.R* script runs the base transmission model to reach a stationary solution. The *main.R* script loads the required packages, sources the required scripts and allows the user the run the models.
